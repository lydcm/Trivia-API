...
def create_app(test_config=None):
    ...
    def get_question_for_quiz():
        ...
        def check_if_question_was_used(quiz_question):
            used = False
            for prev in previous_questions:
                if prev == quiz_question.id:
                    used = True
            return used

        quiz_question = get_random_question()

        while check_if_question_was_used(quiz_question):
            quiz_question = get_random_question()

            if len(previous_questions) == total_questions:
                return jsonify({"success": True})

        return jsonify({"success": True, "question": quiz_question.format()})
